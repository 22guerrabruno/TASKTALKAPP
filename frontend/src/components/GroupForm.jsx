          ...
          {allUsers.map((contact) => (
            <button
              ...
              className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 
                        px-4 m-2 rounded focus:outline-none mt-2 focus:shadow-outline"
            >
              {contact.name}
            </button>
          ))}
        </div>
        <div className="mt-10 bold">Participantes:</div>
        <div className="my-1">
          {names.map((name) => (
            <span
              className="mx-2 py-1 rounded-lg border-2 border-gray-500 p-2"
              key={name}
            >
              {name}
            </span>
